...
fn generic_test(
    ...
    for i in 0..3 {
        ...
        info!("wait for clients");
        for (i, clnt_rx) in clnt_rxs.iter().enumerate() {
            info!("wait for clients {}", i);
            let j = clnt_rx.recv().unwrap();
            info!("wait for clients {} end", i);
            if j < 10 {
                debug!(
            ...
            let v = get(&cfg, &ck, &key);
            info!("start check client");
            check_clnt_appends(i, v, j);
            info!("end check client");
        }
